<?php
	class l6eanalyzecematrixData extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new AnalyzeCeMatrixDataQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new AnalyzeCeMatrixData();
					$new_record->fromArray($args);
					if (!$new_record->getMatrixId() || !$new_record->getMatrixOutputId()){
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('AnalyzeCeMatrixData', $args['filters']);
					$q = AnalyzeCeMatrixDataQuery::create();
					if ($filters){
