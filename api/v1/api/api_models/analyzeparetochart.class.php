<?php
  class l6eanalyzeparetochart extends APIApp{
    protected $response;
    ...
    private function handleGet($args) {
      $location = new AnalyzeParetoChartQuery();
    ...
    private function handlePost($args){
      switch ($this->post_action){
        case 'add':
          $new_record = new AnalyzeParetoChart();
        ...
        case 'update':
          $vals = $args['vals'];
          $filters = $this->formatFilters('AnalyzeParetoChart', $args['filters']);
          $q = AnalyzeParetoChartQuery::create();
          if ($filters){
