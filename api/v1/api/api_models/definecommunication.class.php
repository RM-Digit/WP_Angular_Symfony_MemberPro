<?php
	class l6edefinecommunication extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new DefineCommunicationQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new DefineCommunication();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('DefineCommunication', $args['filters']);
					$q = DefineCommunicationQuery::create();
					if ($filters){
