<?php
	class l6edefinegatereview extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new DefineGateReviewQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new DefineGateReview();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('DefineGateReview', $args['filters']);
					$q = DefineGateReviewQuery::create();
					if ($filters){
