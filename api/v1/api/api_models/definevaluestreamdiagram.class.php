<?php
	class l6edefinevaluestreamdiagram extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new DefineValueStreamDiagramQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new DefineValueStreamDiagram();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('DefineValueStreamDiagram', $args['filters']);
					$q = DefineValueStreamDiagramQuery::create();
					if ($filters){
