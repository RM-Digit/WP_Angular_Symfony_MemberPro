<?php
  class l6emeasurehistogram extends APIApp{
    protected $response;
    ...
    private function handleGet($args) {
      $location = new MeasureHistogramQuery();
    ...
    private function handlePost($args){
      switch ($this->post_action){
        case 'add':
          $new_record = new MeasureHistogram();
        ...
        case 'update':
          $vals = $args['vals'];
          $filters = $this->formatFilters('MeasureHistogram', $args['filters']);
          $q = MeasureHistogramQuery::create();
          if ($filters){
