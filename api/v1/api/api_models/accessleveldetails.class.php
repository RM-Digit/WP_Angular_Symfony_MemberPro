<?php
	class l6eaccessleveldetails extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new AccessLevelDetailsQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new AccessLevelDetails();
					$new_record->fromArray($args);
					if (!$new_record->getAccessLevelId() || !$new_record->getAccessLevelOptionId()){
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('AccessLevelDetails', $args['filters']);
