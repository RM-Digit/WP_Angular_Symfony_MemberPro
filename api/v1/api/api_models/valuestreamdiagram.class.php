<?php
	class l6evaluestreamdiagram extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new ValueStreamDiagramQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new ValueStreamDiagram();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('ValueStreamDiagram', $args['filters']);
					$q = ValueStreamDiagramQuery::create();
					if ($filters){
