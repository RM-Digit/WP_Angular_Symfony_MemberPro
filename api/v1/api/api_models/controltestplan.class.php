<?php
	class l6econtroltestplan extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new ControlTestPlanQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new ControlTestPlan();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('ControlTestPlan', $args['filters']);
					$q = ControlTestPlanQuery::create();
					if ($filters){
