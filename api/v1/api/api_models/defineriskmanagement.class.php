<?php
	class l6edefineriskmanagement extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new DefineRiskManagementQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new DefineRiskManagement();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('DefineRiskManagement', $args['filters']);
					$q = DefineRiskManagementQuery::create();
					if ($filters){
				...
				case 'get':
					...
					foreach ($r AS $k=>$resp){
						$r[$k]['DueDate'] = date('m/d/Y',strtotime($resp['DueDate']));
					}
					$this->response = $r;
					break;
				case 'delete':
