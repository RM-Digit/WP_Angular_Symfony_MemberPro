<?php
  class l6econtrolcontrolplan extends APIApp{
    protected $response;
    ...
    private function handleGet($args) {
      $location = new ControlControlPlanQuery();
    ...
    private function handlePost($args){
      switch ($this->post_action){
        case 'add':
          $new_record = new ControlControlPlan();
        ...
        case 'update':
          $vals = $args['vals'];
          $filters = $this->formatFilters('ControlControlPlan', $args['filters']);
          $q = ControlControlPlanQuery::create();
          if ($filters){
