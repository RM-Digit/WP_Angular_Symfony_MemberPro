<?php
	class l6eimprovevaluestreamdiagram extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new ImproveValueStreamDiagramQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new ImproveValueStreamDiagram();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('ImproveValueStreamDiagram', $args['filters']);
					$q = ImproveValueStreamDiagramQuery::create();
					if ($filters){
