<?php
  class l6econtrolspcchart extends APIApp{
    protected $response;
    ...
    private function handleGet($args) {
      $location = new ControlSpcChartQuery();
    ...
    private function handlePost($args){
      switch ($this->post_action){
        case 'add':
          $new_record = new ControlSpcChart();
        ...
        case 'update':
          $vals = $args['vals'];
          $filters = $this->formatFilters('ControlSpcChart', $args['filters']);
          $q = ControlSpcChartQuery::create();
          if ($filters){
