<?php
	class l6eprojectphases extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new ProjectPhasesQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new ProjectPhases();
					$new_record->fromArray($args);
					if (!$new_record->getProjectId() || !$new_record->getName()){
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('ProjectPhases', $args['filters']);
					$q = ProjectPhasesQuery::create();
					if ($filters){
