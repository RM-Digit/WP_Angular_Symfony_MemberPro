<?php
	class l6econtrolgatereview extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new ControlGateReviewQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new ControlGateReview();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('ControlGateReview', $args['filters']);
					$q = ControlGateReviewQuery::create();
					if ($filters){
