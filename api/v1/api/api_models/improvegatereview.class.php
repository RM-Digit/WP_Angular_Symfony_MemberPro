<?php
	class l6eimprovegatereview extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new ImproveGateReviewQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new ImproveGateReview();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('ImproveGateReview', $args['filters']);
					$q = ImproveGateReviewQuery::create();
					if ($filters){
