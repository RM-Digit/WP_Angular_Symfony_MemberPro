<?php
	class l6eimproveimprovementplan extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new ImproveImprovementPlanQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new ImproveImprovementPlan();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('ImproveImprovementPlan', $args['filters']);
					$q = ImproveImprovementPlanQuery::create();
					if ($filters){
