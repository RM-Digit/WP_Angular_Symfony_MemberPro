<?php
	class l6emeasurevaluestreamdiagram extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new MeasureValueStreamDiagramQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new MeasureValueStreamDiagram();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('MeasureValueStreamDiagram', $args['filters']);
					$q = MeasureValueStreamDiagramQuery::create();
					if ($filters){
