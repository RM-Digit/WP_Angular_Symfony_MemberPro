<?php
	class l6eanalyzecriticalx extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new AnalyzeCriticalXQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new AnalyzeCriticalX();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('AnalyzeCriticalX', $args['filters']);
					$q = AnalyzeCriticalXQuery::create();
					if ($filters){
