<?php
	class l6emeasurecollectionplan extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new MeasureCollectionPlanQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new MeasureCollectionPlan();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('MeasureCollectionPlan', $args['filters']);
					$q = MeasureCollectionPlanQuery::create();
					if ($filters){
