<?php
	class l6eanalyzecematrixOutputs extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new AnalyzeCeMatrixOutputsQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new AnalyzeCeMatrixOutputs();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('AnalyzeCeMatrixOutputs', $args['filters']);
					$q = AnalyzeCeMatrixOutputsQuery::create();
					if ($filters){
