<?php
	class l6edefinevocccr extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new DefineVocCcrQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('DefineVocCcr', $args['filters']);
					$q = DefineVocCcrQuery::create();
					if ($filters){
