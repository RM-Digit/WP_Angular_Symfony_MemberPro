<?php
	class l6emeasurenonvalueanalysis extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new MeasureNonvalueAnalysisQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new MeasureNonvalueAnalysis();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('MeasureNonvalueAnalysis', $args['filters']);
					$q = MeasureNonvalueAnalysisQuery::create();
					if ($filters){
