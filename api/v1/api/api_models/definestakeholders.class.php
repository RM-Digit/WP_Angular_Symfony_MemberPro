<?php
	class l6edefinestakeholders extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new DefineStakeholdersQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new DefineStakeholders();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('DefineStakeholders', $args['filters']);
					$q = DefineStakeholdersQuery::create();
					if ($filters){
