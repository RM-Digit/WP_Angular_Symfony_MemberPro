<?php
	class l6eanalyzegatereview extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new AnalyzeGateReviewQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new AnalyzeGateReview();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('AnalyzeGateReview', $args['filters']);
					$q = AnalyzeGateReviewQuery::create();
					if ($filters){
