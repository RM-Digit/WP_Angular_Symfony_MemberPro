<?php
	class l6emeasuregatereview extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new MeasureGateReviewQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new MeasureGateReview();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('MeasureGateReview', $args['filters']);
					$q = MeasureGateReviewQuery::create();
					if ($filters){
