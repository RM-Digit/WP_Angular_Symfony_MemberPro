<?php
	class l6eanalyzecematrix extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new AnalyzeCeMatrixQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new AnalyzeCeMatrix();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('AnalyzeCeMatrix', $args['filters']);
					$q = AnalyzeCeMatrixQuery::create();
					if ($filters){
