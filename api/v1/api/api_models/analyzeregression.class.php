<?php
  class l6eanalyzeregression extends APIApp{
    protected $response;
    ...
    private function handleGet($args) {
      $location = new AnalyzeRegressionQuery();
    ...
    private function handlePost($args){
      switch ($this->post_action){
        case 'add':
          $new_record = new AnalyzeRegression();
        ...
        case 'update':
          $vals = $args['vals'];
          $filters = $this->formatFilters('AnalyzeRegression', $args['filters']);
          $q = AnalyzeRegressionQuery::create();
          if ($filters){
