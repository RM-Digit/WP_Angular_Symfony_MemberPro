<?php
	class l6eanalyzehypothesismap extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new AnalyzeHypothesisMapQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new AnalyzeHypothesisMap();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('AnalyzeHypothesisMap', $args['filters']);
					$q = AnalyzeHypothesisMapQuery::create();
					if ($filters){
