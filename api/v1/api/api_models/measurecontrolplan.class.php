<?php
	class l6emeasurecontrolplan extends APIApp{
		protected $response;
		...
		private function handleGet($args) {
			$location = new MeasureControlPlanQuery();
		...
		private function handlePost($args){
			switch ($this->post_action){
				case 'add':
					$new_record = new MeasureControlPlan();
				...
				case 'update':
					$vals = $args['vals'];
					$filters = $this->formatFilters('MeasureControlPlan', $args['filters']);
					$q = MeasureControlPlanQuery::create();
					if ($filters){
